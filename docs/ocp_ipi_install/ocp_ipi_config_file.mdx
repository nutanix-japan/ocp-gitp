...
5. Add the details:
    ...
    - architecture: amd64
      ...
      name: worker
      platform: {}
      replicas: 2                                                         ## << This can be also 3 or more nodes
    controlPlane:
      architecture: amd64
      hyperthreading: Enabled
      name: master
    ...
    platform:
    nutanix:
       apiVIP: 10.0.0.1                                                   ## << Change this to your api IP you reserved earlier
       ingressVIP: 10.0.0.2                                               ## << Change this to your Ingress IP you reserved earlier
       prismCentral:
          endpoint:
...
7. Check the contents of ``manifests`` and ``openshift`` directories to make sure all the files are present (including iSCSI daemon MCO config. files)
   ...
   ```
8. Confirm the contents of the ``manifests/cluster-proxy-01-config.yaml`` file to check whether the ``user-ca-bundle`` and the ``rootCA.crt``certificate that you created earlier in the lab is being used.
